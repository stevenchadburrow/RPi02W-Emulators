...
int main()
{
	system("echo \"0000000000000\" > /home/username/LakeSnes/keyboard.val");
	...
	system("lsusb | grep -i 'key' > /home/username/LakeSnes/usb.val");

	FILE *input = NULL;

	input = fopen("/home/username/LakeSnes/usb.val", "rt");
	if (!input)
	{
	...
	system("rm /home/username/LakeSnes/usb.val");
	...
	if (keys_fd > 0)
	{
		while (1)
		{
			...
			sprintf(string, "echo \"%c%c%c%c%c%c%c%c%c%c%c%c%c\" > /home/username/LakeSnes/keyboard.val",

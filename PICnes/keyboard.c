...
int main()
{
	system("echo \"0000000000000\" > /home/username/PICnes/keyboard.val");
	...
	system("lsusb | grep -i 'key' > /home/username/PICnes/usb.val");

	FILE *input = NULL;

	input = fopen("/home/username/PICnes/usb.val", "rt");
	if (!input)
	{
	...
	system("rm /home/username/PICnes/usb.val");
	...
	if (keys_fd > 0)
	{
		while (1)
		{
			...
			sprintf(string, "echo \"%c%c%c%c%c%c%c%c%c%c%c%c%c\" > /home/username/PICnes/keyboard.val",

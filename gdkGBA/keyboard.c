...
int main()
{
	system("echo \"0000000000000\" > /home/username/gdkGBA/keyboard.val");
	...
	system("lsusb | grep -i 'key' > /home/username/gdkGBA/usb.val");

	FILE *input = NULL;

	input = fopen("/home/username/gdkGBA/usb.val", "rt");
	if (!input)
	{
	...
	system("rm /home/username/gdkGBA/usb.val");
	...
	if (keys_fd > 0)
	{
		while (1)
		{
			...
			sprintf(string, "echo \"%c%c%c%c%c%c%c%c%c%c%c%c%c\" > /home/username/gdkGBA/keyboard.val",
